...
export const useChatEngine = ({
        ...
        try {
            const res = await fetch("/api/chat", {
                ...
                body: JSON.stringify({
                    ...
                    videoModel: configOverride?.videoModel || config.videoModel || "hailuo",
            ...
            if (titleMatch && (currentSession.title === "新篇章" || currentSession.title === "New Dream")) {
    ...
    const handleVisualCommand = async (cmdOrPrompt: any) => {
        ...
        if (typeof cmdOrPrompt === 'string') {
            cmd = parseMediaTag(cmdOrPrompt) || { description: cmdOrPrompt, type: 'image', trigger: true };
        }
        if (!cmd || !cmd.trigger) return;

        let targetFingerprint = "";
        const lastMsg = currentSession?.messages.filter((m: any) => m.role === 'assistant').pop();
        if (lastMsg) {
        ...
        try {
            if (cmd.type === 'video' && config.videoGen) {
                const currentVideoModel = cmd.videoModel || config.videoModel || 'hailuo';
                showToast(`🎬 Action! Engine: ${currentVideoModel}`);
                        ...
                        const newMessages = s.messages.map((m: any) => {
                            if (typeof m.content === 'string') {
                                if (targetFingerprint && m.content.includes(targetFingerprint)) {
                                    ...
                                    return { ...m, content: m.content.replace(targetFingerprint, logTag + showcase) };
                                } else if (m === s.messages[s.messages.length - 1]) {
                                    return { ...m, content: m.content + `\n\n![Video](${videoUrl})\n` };
                                }
                            }
