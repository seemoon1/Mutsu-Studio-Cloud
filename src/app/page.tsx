...
export default function Home() {
    ...
    const [videoModel, setVideoModel] = useState<'hailuo' | 'doubao'>('hailuo');
    const [drawEngine, setDrawEngine] = useState<'volcengine' | 'openrouter'>('volcengine');
    ...
    const [apiProvider, setApiProvider] = useState<'deepseek' | 'openrouter' | 'google'>('deepseek');
    ...
    const handleExportData = () => {
        const d = {
            ...
            globalWorldInfo: cloud.globalWorldInfo,
            bgImage: cloud.bgImage
        };
        const b = new Blob([JSON.stringify(d)], { type: "application/json" });
        const a = document.createElement('a');
        a.href = URL.createObjectURL(b);
        a.download = "mutsu_cloud_backup.json";
        a.click();
    };
        ...
        >
            ...
            <div className="relative z-10 flex w-full h-full">
                ...
                <SidebarLeft
                    ...
                    useVideoGen={config.videoGen}
                    toggleVideoGen={() => {
                        setConfig(prev => {
                            const next = !prev.videoGen;
                            ui.showToast(next ? "🎥 Video Mode: ON (High Cost!)" : "⏹ Video Mode: OFF");
                            return { ...prev, videoGen: next };
                        });
                    }}
                    videoModel={videoModel}
                    setVideoModel={setVideoModel}
