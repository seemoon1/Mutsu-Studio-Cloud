...
`;
      ...
      memberIds.forEach((mid: string) => {
        if (mid === "user") {
          groupLore += `--- [${playerName}] ---\nThis participant is controlled directly by the human user. Do not invent dialogue for them.\n`;
          return;
        }
...
The human user (charId "user", display name "${playerName}") IS AN ACTIVE PARTICIPANT in this chat.
The user's input is their actual chat message sent to the chat.
...
`;
        ...
        finalSystemPrompt += `
This is a 1-on-1 private chat between the human user "${playerName}" and ${otherName}.
Only ${otherName} should be generated by the model. The user speaks only through real user input.
`;
      } else {
