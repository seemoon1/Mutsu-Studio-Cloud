...
`;
      ...
      const chatPov = body.limePov === "insider" ? "insider" : "outsider";
      ...
      const playerName = typeof body.limePlayerName === "string" && body.limePlayerName.trim()
        ? body.limePlayerName.trim().slice(0, 24)
        : "玩家";
      ...
      memberIds.forEach((mid: string) => {
        if (mid === "user") {
          groupLore += `--- [${playerName}] ---\nThis participant is controlled directly by the human player. Do not invent dialogue for them.\n`;
          return;
        }
...
The human player (charId "user", display name "${playerName}") IS AN ACTIVE PARTICIPANT in this chat.
The player's input is their actual chat message sent to the chat.
You must generate responses from the OTHER members reacting naturally to the player's message and to each other.
NEVER generate a message with charId "user"; that participant is controlled by the human.
`;
      }

      if (groupType === "duo" && chatPov === "outsider") {
        const povName =
...
`;
      } else if (groupType === "duo") {
        const otherCharId = memberIds.find((id: string) => id !== "user");
        const otherName = CHAR_DATA.find((c) => c.id === otherCharId)?.name || "The Other Character";
        finalSystemPrompt += `
This is a 1-on-1 private chat between the human player "${playerName}" and ${otherName}.
Only ${otherName} should be generated by the model. The player speaks only through real user input.
`;
      } else {
